...
Additionally, sub-agents need **structural understanding** of the codebase. GitNexus provides this via its Knowledge Graph — relational intelligence (calls, imports, processes) that replaces blind file-by-file exploration.
...
## GitNexus as Context Source

Before delegating code-related work, check GitNexus for structural context:

1. **`gitnexus_query`** — find execution flows related to the task
2. **`gitnexus_context`** — understand callers/callees of target symbols
3. **`gitnexus_impact`** — assess blast radius before the sub-agent starts

Pass relevant GitNexus findings in the sub-agent prompt alongside skill rules. This gives the sub-agent both **standards** (skills) and **structural awareness** (GitNexus).

See `skills/_shared/gitnexus-protocol.md` for the full tool catalog and usage rules.
