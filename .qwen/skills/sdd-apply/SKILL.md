...
#### Step 2a: Structural Understanding via GitNexus

**Cargá el contexto del diseño. Implementá siguiendo el plano. No inventes arquitectura.**

```
GITNEXUS PRE-IMPLEMENTATION:
├── 1. gitnexus_context(name: "symbol_to_modify")
│     → 360° view: understand current patterns, callers, callees
│     → Know exactly what you're touching and who depends on it
├── 2. gitnexus_query("related functionality or similar implementations")
│     → Find existing similar code to follow the same patterns
│     → Match project conventions automatically
└── 3. Read specific files ONLY for implementation details
      → Targeted reads for exact code you'll reference
```

**Si detectás una discrepancia con el grafo de GitNexus mientras escribís, detené la ejecución y pedí feedback al Orquestador.**

After GitNexus results, complete context reading:
3. Read existing code in affected files — verify current patterns match graph
