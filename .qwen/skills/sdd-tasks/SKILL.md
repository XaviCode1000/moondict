...
### Step 2: Analyze the Design with GitNexus Dependency Tracing
...
```
GITNEXUS DEPENDENCY ORDERING:
├── 1. gitnexus_impact(target: "new_or_modified_symbol", direction: "downstream")
│     → Understand what the symbol depends on (its upstream dependencies)
│     → This tells you what must be implemented FIRST
├── 2. gitnexus_query("process or flow for {domain}")
│     → Understand execution flow order
│     → Tasks should follow the natural process flow
└── 3. Order: Interfaces → Implementations → Callers → Tests
      (This order is enforced by the dependency graph)
```
