...
### Step 1b: GitNexus Index Check

**Verificá si el búnker está preparado.**

```
GITNEXUS INDEX STATUS:
├── 1. gitnexus_list_repos()
│     → Check if this project is indexed in GitNexus
├── 2. If project IS indexed:
│     → Cache repo context for subsequent phases
│     → Note the indexed path and last commit
├── 3. If project is NOT indexed (or stale):
│     → RECOMMEND to user: "npx gitnexus analyze --embeddings"
│     → Flag: The graph won't be available for subsequent SDD phases
│     → Proceed with file-based detection (fallback)
└── 4. If indexed but stale (graph != HEAD):
      → RECOMMEND: "npx gitnexus analyze --embeddings" to refresh
      → Note: embeddings flag is critical to preserve vector search
```

**Identificá Hotspots arquitectónicos** (si GitNexus está disponible):
- Comunidades de mayor acoplamiento
- Símbolos con más callers (high blast radius targets)
- Process flows that are entry points to the system
