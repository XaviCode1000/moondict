...
**Tu tablero es el Knowledge Graph.** Antes de diseñar, entendé la arquitectura real:

```
GITNEXUS DESIGN INTELLIGENCE:
├── 1. gitnexus_query("existing patterns for {domain}")
│     → Understand current architecture and conventions
│     → Find similar implementations to follow
├── 2. gitnexus_context(name: "entry_point_symbol")
│     → 360° view of entry points: callers, callees, dependencies
│     → Understand where the new code will plug in
├── 3. gitnexus_route_map() — IF API changes
│     → Understand API consumption patterns
│     → See which consumers fetch which endpoints
└── 4. Read specific files ONLY for details not in the graph
      → Targeted reads based on graph results
```
...
- ALWAYS use GitNexus to understand the actual codebase before designing — never guess
