...
### Step 1b: GitNexus Change Detection (Structural Integrity)

**Ejecutá `detect_changes` para mapear tu git diff contra los flujos de negocio.**

```
GITNEXUS POST-IMPLEMENTATION SCAN:
├── 1. gitnexus_detect_changes()
│     → Maps uncommitted git diffs to affected symbols
│     → Traces which execution processes are impacted
│     → Returns risk summary (changed symbols, affected processes)
├── 2. Review affected symbols and processes
│     → Cross-reference against spec requirements
│     → Ensure no "Blind Edits" outside approved blast radius
└── 3. If unexpected symbols appear:
      → STOP — you changed more than planned
      → Report to orchestrator before continuing
```

This is your first line of defense against blind edits. Run this BEFORE any other verification step.
