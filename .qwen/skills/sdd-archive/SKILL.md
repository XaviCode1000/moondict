...
### Step 1b: GitNexus Status Check

**Verificá que el índice de GitNexus coincida con HEAD antes de archivar.**

```
GITNEXUS PRE-ARCHIVE CHECK:
├── 1. gitnexus_list_repos() — confirm project is indexed
├── 2. If project indexed:
│     → The graph should reflect the code you just implemented
│     → If the graph is stale, recommend re-index after commit
└── 3. If project NOT indexed:
      → Note in archive report: "GitNexus index not available"
      → Recommend: npx gitnexus analyze --embeddings
```

This ensures the Knowledge Graph reflects your changes before the change is archived.
